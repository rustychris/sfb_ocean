# 02: use grid-merge-suisun, instead of the older grid-merged.
...
ccoll=grid_ocean.plot_cells(ax=ax,values=(-grid_ocean.cells['z_bed']).clip(1,np.inf),
                            norm=colors.LogNorm(),
                            cmap=cmap)
...
grid_ocean.write_cells_shp("../gis/merge-suisun-bathy.shp",
                           extra_fields=[ ('depth',(-grid_ocean.cells['z_bed']).clip(1,np.inf)),
                                          ('elev',grid_ocean.cells['z_bed'].clip(-np.inf,-1)),
                                          ('logdepth',np.log10( (-grid_ocean.cells['z_bed']).clip(1,np.inf))),
                                          ('normed',normed)
                           ],
                           overwrite=True)
##
